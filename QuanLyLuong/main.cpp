...
{
	// Ki tu dau tien se duoc viet hoa 
	ten[0] = toupper(ten[0]);
	// Ki tu tiep theo se duoc viet thuong 
...
void chuanHoaTen1(string& hoTenNhanVien)
{
	...
	// Bo cac khoang trang o dau chuoi 
	while (hoTenNhanVien[0] == ' ')
	{
		hoTenNhanVien.erase(hoTenNhanVien.begin() + 0);
	}// Tach ngan tu chuoi ho ten nhan vien 
	...
	// Truyen ten ngan vua tach vao chuanHoaTen2 
...
bool kiemTraTrungMa(const string& ma, const vector<Employee*>& danhSachNhanVien) {
	for (const auto& nhanVien : danhSachNhanVien) {
...
bool isValidNgayVaoLam(const std::string& ngayVaoLam) {
	...
	// So sánh ngày vào làm v?i th?i gian hi?n t?i
	tm now_tm;
...
void themNhanVien(string& maNhanVien, string& hoTenNhanVien, string& ngayVaoLam, string& soDienThoai, int& luongCanBan) {
	...
	do {
		...
		if (kiemTraTrungMa(maNhanVien, danhSachNhanVien)) {
			cout << "Ma nhan vien da ton tai. Vui long nhap lai!!" << endl;
		}
	} while (kiemTraTrungMa(maNhanVien, danhSachNhanVien));
	chuanHoaTen2(maNhanVien);
...
void demNhanVienVaoLamTrongNam(const vector<Employee*>& danhSachNhanVien) {
	...
	for (int i = 0; i < danhSachNhanVien.size(); i++) {
		if (FullTimeEmployee* fullTime = dynamic_cast<FullTimeEmployee*>(danhSachNhanVien[i])) {
			// chuyen tu string sang int lay tu ki tu vi tri 6 4 ki tu 
...
void menu() {
	cout << "       QUAN LI LUONG CHO NHAN VIEN  \n";
	cout << endl;
	cout << "-----------------MENU-----------------" << endl;
